...
export default function MyRefuse() {
    ...
    useEffect(() => {
        axios.get(`http://localhost:${myPort}/auth/approval/myrefuse`, { headers: { Authorization: token } })
            .then(
                function (res) {
                    if (res.status === 200) {
                    ...
                    <div className="col-md-12">
                        <div className="card">
                            <div className="card-header card-header-danger">
                                <h4 className="card-title">반려된 문서</h4>
                            </div>
                                    ...
                                    <tbody>
                                        {elist.map((e) => (
                                            <tr key={e.expenseNum}>
                                                ...
                                                <td>{e.member.name}</td>
                                                <td>
                                                <Link to={`/approval/expense/editread/${e.expenseNum}`}>{e.title}</Link>
                                                </td>
                                                <td>{e.wdate}</td>
                                        ...
                                        {vlist.map((v) => (
                                            <tr key={v.vacationNum}>
                                                ...
                                                <td>{v.member.name}</td>
                                                <td>
                                                <Link to={`/approval/vacation/editread/${v.vacationNum}`}>{v.member.name} 휴가신청</Link>
                                                </td>
                                                <td>{v.wdate}</td>
