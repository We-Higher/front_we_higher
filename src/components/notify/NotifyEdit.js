...
export default function NotifyEdit() {
    ...
    useEffect(() => {
        axios.get(`http://localhost:${myPort}/auth/notify/edit/` + n, { headers: { Authorization: token } })
            .then(function (res) {
                if (res.status === 200) {
                    let b = res.data.dto;
                    setDto({
    ...
    const edit = () => {
        axios.put(`http://localhost:${myPort}/auth/notify/edit`,
            {},
